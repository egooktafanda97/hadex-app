...
export async function saveTripAction(_: ActionResult | undefined, form: FormData) {
  try {
    const u = await admin();
    saveTrip(u.id, value(form, "id") || undefined, {
      tripCode: value(form, "tripCode"),
      busId: value(form, "busId"),
      routeId: value(form, "routeId"),
      departureAt: toIsoDateTime(value(form, "departureAt")),
      arrivalAt: toIsoDateTime(value(form, "arrivalAt")),
      fare: value(form, "fare"),
      status: value(form, "status") || undefined,
    });
    revalidatePath("/admin/trips");
    return { ok: true, message: "Trip tersimpan" };
  } catch (e) {
    return result(e);
  }
}
